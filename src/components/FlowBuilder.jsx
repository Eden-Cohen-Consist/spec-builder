...
function Connector() {
  return (
    <div className="flex flex-col items-center py-1" aria-hidden="true">
      ...
      <ChevronDown strokeWidth={2.5} className="-mt-2 size-3.5 text-stone-300" />
    </div>
  )
}
...
function SplitConnector() {
  return (
    <div className="relative h-9" aria-hidden="true">
      <div className="absolute right-1/2 top-0 h-3.5 w-px bg-stone-300" />
      <div className="absolute left-1/4 right-1/4 top-3.5 h-px bg-stone-300" />
      <div className="absolute left-1/4 top-3.5 h-[22px] w-px bg-stone-300" />
      <div className="absolute right-1/4 top-3.5 h-[22px] w-px bg-stone-300" />
    </div>
  )
}
...
function StepNode({ node, number, deletable, onUpdate, onDelete }) {
  return (
    <div className="group animate-block-in rounded-xl border border-stone-200 bg-white p-4 shadow-[0_1px_2px_rgba(28,25,23,0.03)] transition-colors duration-150 hover:border-stone-300">
      <div className="mb-2 flex items-center justify-between">
        <span className="inline-flex items-center rounded-md bg-teal-700/8 px-2 py-0.5 text-[12px] font-bold text-teal-800">
          שלב {number}
        </span>
        {deletable && (
          <DeleteButton
      ...
      <textarea
        ...
        className="w-full resize-none bg-transparent text-[15px] leading-relaxed text-ink outline-none placeholder:text-stone-400"
      />
    </div>
  )
}
  ...
  return (
    <div className="group animate-block-in">
      <div className="relative flex items-center justify-center">
        <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-200/80 bg-amber-50 px-3 py-1 text-[12px] font-bold text-amber-800">
      ...
      <div className="grid grid-cols-2 gap-4">
        {node.paths.map((path, i) => (
          <div
            key={i}
            className="rounded-xl border border-stone-200 bg-white p-4 shadow-[0_1px_2px_rgba(28,25,23,0.03)] transition-colors duration-150 hover:border-stone-300"
          >
            <div className="mb-2 text-[11.5px] font-bold uppercase tracking-wide text-stone-400">
              {PATH_LABELS[i]}
            </div>
            <Input
              value={path.name}
            ...
            <textarea
              ...
              className="w-full resize-none bg-transparent text-[14px] leading-relaxed text-ink outline-none placeholder:text-stone-400"
            />
          </div>
        ))}
      </div>
...
export default function FlowBuilder({ flow, onChange, delay }) {
  ...
  return (
    <LockedSection number="3" title="תהליך לוגי — מבט על" subtitle="השלבים העסקיים של התהליך, מהטריגר ועד הסוף" delay={delay}>
      <div className="rounded-xl bg-stone-50/60 p-4 ring-1 ring-inset ring-stone-100">
        {flow.map((node, index) => {
