...
import { isThirdParty, makeMappingRow } from '../lib.js'
    ...
    <Field label={<span dir="ltr">{label}</span>} hint={invalid ? 'שדה חובה' : undefined}>
      <textarea
        ...
        className={`code-scroll w-full resize-y rounded-lg bg-[#161412] p-3.5 text-left font-mono text-[12.5px] leading-relaxed text-stone-200 caret-teal-400 outline-none ring-1 transition-shadow duration-150 placeholder:text-stone-600 ${
          invalid ? 'ring-2 ring-red-500' : 'ring-stone-700/80 focus:ring-2 focus:ring-teal-600'
        }`}
      />
    </Field>
  )
...
function MappingTable({ mapping, onChange }) {
  ...
  return (
    <div className="mt-5">
      <h4 className="mb-2 text-[13.5px] font-bold text-stone-700">מיפוי שדות (Data Mapping)</h4>
                ...
                <td>
                  <span className="relative block">
                    <select
                      ...
                      className="w-full cursor-pointer appearance-none bg-transparent py-2.5 pe-7 ps-3 text-[13px] text-stone-700 outline-none transition-colors focus:bg-teal-600/5"
                    >
                    ...
                    <ChevronDown className="pointer-events-none absolute end-2 top-1/2 size-3.5 -translate-y-1/2 text-stone-400" />
                  </span>
                </td>
  ...
  return (
    <div>
      <div className="grid grid-cols-[1fr_auto_1fr] items-end gap-3">
        ...
        <MoveLeft aria-hidden="true" className="mb-2.5 size-5 text-stone-300" />
        <Field
          label={
            <span className="flex items-center gap-2">
              מערכת יעד
              {thirdParty && (
                <span className="animate-pop inline-flex items-center gap-1 rounded-full border border-amber-200/80 bg-amber-50 px-2 py-px text-[11px] font-bold text-amber-700">
      ...
      {thirdParty && (
        <div className="animate-block-in mt-5 space-y-4 rounded-xl border border-stone-200 bg-stone-50/60 p-4">
