...
export default function BusinessSection({ value, onChange, delay }) {
  ...
  const setTrigger = (id, text) =>
    set({ triggers: value.triggers.map((t) => (t.id === id ? { ...t, text } : t)) })
  const removeTrigger = (id) => set({ triggers: value.triggers.filter((t) => t.id !== id) })
  const addTrigger = () => set({ triggers: [...value.triggers, makeTriggerRow()] })
      ...
      <div className="space-y-4">
        ...
        <div>
          <span className="mb-1.5 flex items-baseline gap-2 text-[13px] font-semibold text-stone-600 dark:text-stone-300">
            ...
            <span className="font-normal text-stone-400 dark:text-stone-500">למשל: "בעת סגירת טיקט"</span>
          </span>
          <div className="space-y-2">
            {value.triggers.map((trigger, index) => (
              <div key={trigger.id} className="group/trigger flex items-center gap-1.5">
                <Input
                  value={trigger.text}
                  onChange={(e) => setTrigger(trigger.id, e.target.value)}
                  placeholder={index === 0 ? 'מה בדיוק מפעיל את התהליך?' : 'טריגר נוסף...'}
                />
