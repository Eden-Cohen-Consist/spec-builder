...
import { toClaudeStreamParams, toUsageTurn } from "../transformers/claudeTransformer.js";
...
export async function* streamChat( request: ChatRequest,signal?: AbortSignal ): AsyncGenerator<ChatStreamEvent> {
  ...
  logger.info("Claude stream completed", { module: "streamChat", sessionId: request.sessionId });

  const usageTurn = toUsageTurn(
    request,
    claudeStreamParams,
    message,
    toolBlocks,
    stream.request_id ?? null,
    startedAt,
  );
